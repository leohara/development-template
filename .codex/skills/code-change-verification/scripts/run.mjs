...
Usage:
  node .codex/skills/code-change-verification/scripts/run.mjs
`);
}
...
function createCommandStep(label, command, args) {
  return {
    label,
    command,
    args,
...
function createPnpmStep(label, args) {
  return createCommandStep(label, getPnpmCommand(), args);
}

export function createDefaultPlan() {
  return {
    sequentialSteps: [
      createPnpmStep("lint", ["run", "lint"]),
      createPnpmStep("type-check", ["run", "type-check"]),
      createPnpmStep("test", ["run", "test"]),
      createPnpmStep("build", ["run", "build"]),
    ],
    parallelSteps: [],
  };
}
...
function startStep(step, repoRoot) {
  ...
  const stdoutDone = forwardPrefixedOutput(child.stdout, process.stdout, step.label);
  const stderrDone = forwardPrefixedOutput(child.stderr, process.stderr, step.label);
...
async function runParallelSteps(steps, repoRoot, activeRuns) {
  if (steps.length === 0) {
    return { exitCode: 0, failedStep: null, results: [] };
  }
  ...
  if (outcome.type === "done") {
    const failedResult = outcome.results.find((result) => result.exitCode !== 0);
    for (const run of runs) {
      activeRuns.delete(run);
    }
    if (failedResult) {
...
export async function runVerification(options = {}) {
  ...
  const sequentialSteps = options.sequentialSteps ?? defaultPlan.sequentialSteps;
  ...
  const handleSignal = async (signal) => {
    ...
    interrupted = true;
    console.error(`code-change-verification: received ${signal}. Stopping running steps.`);
    await Promise.allSettled(
      [...activeRuns].map((run) => terminateRun(run, process.platform === "win32")),
    );
  ...
  try {
    ...
    const parallelResult = await runParallelSteps(parallelSteps, repoRoot, activeRuns);
